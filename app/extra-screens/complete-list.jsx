...
const CompleteList = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const completedTasks = fetchedTasks.filter((task) => task.status); // Filter only completed tasks
      setTasks(SortTasks(completedTasks, sortBy)); // Sort tasks after fetching
    } catch (error) {
    ...
    <SafeAreaView className="bg-primary h-full">
      ...
      {tasks.length === 0 ? (
        <EmptyState 
          ...
          subtitle="There are no tasks that have been completed yet."
        />
      ) : (
        <FlatList
          data={tasks}
